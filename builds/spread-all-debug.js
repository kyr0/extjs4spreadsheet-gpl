/*! Ext JS 4 SpreadSheets - v0.1 - 2012-12-06
* http://www.extjs4spreadsheets.com/
* Copyright (c) 2012 Copyright (C) 2012, Aron Homberg; GPLv3 and commercially licensed. */

/*! tea.js - TEstAble JS - v0.1 - 2012-11-01
* http://www.teajs.org/
* Copyright (c) 2012 Copyright (C) 2012, Aron Homberg; MIT licensed. */

/**
 * Test class
 * @param {Function|String} fn Function reference of method name
 * @param {Arguments} [args] Function call arguments
 * @return {Function}
 */
var T = function(fn, args) {

    // Auto-enable
    T.enabled = true;

    // Test inside method
    if (args && fn && typeof fn != 'function') {

        return T.letsTest(fn, args);

    } else if (typeof fn == 'function' ) {

        // Return an occupied function
        return T.occupy(fn);

    } else {

        throw "Function must be given or method name with arguments!";
    }
};

/**
 * En-/Disable testing (auto-enabled so this method is for disabling primarily)
 * @param {Boolean} shouldBeEnabled=true Should testing be enabled?
 * @return {T}
 */
T.setEnabled = function(shouldBeEnabled) {

    if (typeof shouldBeEnabled == 'undefined') {
 ...
 * @return {Function}
 */
T.occupy = function(fn) {

    // Break, if disabled
    if (!T.enabled) return fn;

    // Return the occupied function
    var occupyFn = function() {

        if (!fn._T_testable) {
            T.makeTestable(fn);
        }

        // Register a function call start
        T.Reporter.callStart(fn);

        // Fetch result
        var result = fn.apply(this, arguments);

        // Register a function call end
        T.Reporter.callEnd(fn);

        return result;
    }

    // Register a function to test
    T.Reporter.registerFn(fn, occupyFn);

    return occupyFn;
};

/**
 * Returns the function reference itself for given arguments
 * @param {Arguments} args Function arguments
 * @return {Function}
 */
T.letsTest = function(methodName, args) {

    // Break if disabled
    if (!T.enabled) return args.callee;

    if (!methodName) {
        throw "How should I test you, when I don't know your name, baby? Try: T.letsTest('myMethodName', arguments);";
    }

    if (!args) {
        throw "Common, without any good arguments I won't work for you! Try: T.letsTest('myMethodName', arguments);";
    }

    // Set test name locally
    args.callee._T_testName = methodName;

    return args.callee;
};

/**
 * Makes origin functions testable by assigning testing functions
 * to their instances - simple and straight forward!
 * @param {Function} originFn Origin function reference
 * @return void
 */
T.makeTestable = function(originFn) {

    /**
     * Personal assertion function
     * @param {*} value Value to assert
     * @param {String} [title=auto-generated] Description of whats getting asserted in natural language
     * @return {T.Assertable}
     */
    originFn.assert = function(value, title) {

        var assertable = new T.Assertable(originFn, value, title);
            assertable.not = new T.Assertable(originFn, value, title, true);

        return assertable;
    };

    // Set function-instance flag
    originFn._T_testable = true;
};

/**
 * Allows to assert without testing.
 * Use this for type safety checks.
 * @param {*} value Value to assert
 * @param {String} [title=auto-generated] Description of whats getting asserted in natural language
 * @return {T.Assertable}
 */
T.assert = function(value, title) {
    var assertable = new T.Assertable(this, value, title);
    assertable.not = new T.Assertable(this, value, title, true);
    return assertable;
};
/**
 * Reporter
...
T.Reporter = {
    ...
    /**
     * @prop {Array} registeredFns
     * Functions registered for testing
     */
    registeredFns: [],

    /**
     * Register a assertion call
     * @param assertion
     */
    asserted: function(assertion) {
        console.log('Asserted: ', assertion);
    },

    /**
     * Register a call start
     * @param originFn
     * @param occupiedFn
     */
    callStart: function(originFn) {
        console.log('Start call of function: ', originFn);
    },
     ...
     * @param occupiedFn
     */
    callEnd: function(originFn) {
        console.log('End call of function: ', originFn);
    },

    /**
     * Registers an occupied test function
     * @param {Function} originFn Original test function reference
     * @param {Function} occupiedFn Occupied function reference
     * @return void
     */
    registerFn: function(originFn, occupiedFn) {

        console.log('Registered a function for testing: ', originFn, occupiedFn);

        // Push function onto internal stack
        T.Reporter.registeredFns.push({
            originFn: originFn,
            occupiedFn: occupiedFn
        });
    }
};
T.Assertable = function(originFn, value, title, negate) {
    ...
    this.value = value;
    this.negate = negate;
    this.assertFn = 'unknown';
};

/**
 * Returns a nice natural language title for each assertion
 * @return {String}
 */
T.Assertable.prototype.getTitle = function() {
    return this.title ? this.title : this.generateTitle();
};

/**
 * Generates a natural language title because no title was
 * given while assertion.
 * @return {String}
 */
T.Assertable.prototype.generateTitle = function() {
    return 'TODO';
};

/**
 * Asserts the given value to be defined
 * @return {Boolean}
 */
T.Assertable.prototype.toBeDefined = function() {

    // Break, if disabled
    if (!T.enabled) return false;

    var r = typeof this.value != 'undefined';
        r = this.negate ? !r : r;

    // Set result value
    this.result = r;

    // Set assert method name
    this.assertFn = 'toBeDefined';

    // Inform the reporter about a assertion call
    T.Reporter.asserted(this);
